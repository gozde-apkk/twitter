...
const Footer = () => {
    return (
        <footer className='mb-4 px-4 flex flex-wrap gap-2'>
            ...
            <Link to='/' className='text-[color:var(--color-base-secondary)] leading-4 text-[13px]  hover:underline '>Çeraz Politikası</Link>
                ...
                <Popover.Button
                 className='text-[color:var(--color-base-secondary)] outline-none leading-4 text-[13px]  hover:underline inline-flex items-center '>
                    ...
                    <Popover.Panel className="w-[150px] max-w-[384px] bg-[color:var(--background-primary)]shadow-box rounded-xl absolute bottom-0 right-0 grid overflow-hidden ">
                        <Link to='/'className='py-3 px-4 text-[15px] text-white  leading-5 font-bold hover:bg-[color:var(--background-secondary)]transition-colors' >Hakkında</Link>
                        <Link to='/'className='py-3 px-4 text-[15px] text-white  leading-5 font-bold hover:bg-[color:var(--background-secondary)]transition-colors' >Durum</Link>

                        <Link to='/'className='py-3 px-4 text-[15px] text-white  leading-5 font-bold hover:bg-[color:var(--background-secondary)]transition-colors' >İşletmeler için X</Link>
                        <Link to='/'className='py-3 px-4 text-[15px] text-white  leading-5 font-bold hover:bg-[color:var(--background-secondary)]transition-colors' >Geliştiriciler</Link>

                    </Popover.Panel>
            </Popover>
            <p className='text-[color:var(--color-base-secondary)] leading-4 text-[13px] '> © 2023 X Corp.</p>
           </div>
        </footer>
    );
}
